...
export const VariantDiscounts = ({ discounts = [], onChange }: Props) => {
  const addDiscount = () => {
    onChange([...discounts, { code: "", type: "PERCENTAGE", value: 0, percentage: 0 }]);
  };
  ...
  return (
    <div className="space-y-4 bg-green-50/50 p-4 rounded-lg">
      <div className="flex justify-between items-center">
        ...
        <Button variant="naked" size="sm" type="button" onClick={addDiscount} color="success">
      ...
      {discounts.map((disc, index) => (
        <div className="flex gap-2 items-center" key={index}>
          ...
          />
          <Button variant="naked" color="danger" onClick={() => onChange(discounts.filter((_, i) => i !== index))}>
            <Trash2 size={16} />
          </Button>
        </div>
      ))}
