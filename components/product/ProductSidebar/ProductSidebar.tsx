...
const ProductSidebar: FC<ProductSidebarProps> = ({ product, className, variant, setVariant }: ProductSidebarProps) => {
  ...
  const price = calculatePrice(variant.price, variant.currency, [{ type: 'FIXED_AMOUNT', value: 10 }])
  ...
  return (
    <div className={cn(className, 'space-y-8')}>
      ...
      {product.categories.map((category) => <span key={category.id} className={'text-xl mr-2'}>{category.name}</span>)}
      ...
      <section>
        ...
        <p className='text-lg font-medium  space-x-2 my-2'>
          ...
          {price.hasDiscount ?
            <>
              <span className="line-through opacity-40">{price.priceCalculated}</span>
              <span>{price.priceDiscount}</span>

              <span className=' bg-red-600 p-1 px-2 border-2 border-accent-9 bg-center'>SALE</span>
            </>
        ...
        <p className='text-sm'>VAT included for EU orders. Duties and import taxes are calculated at checkout for U.S. customers Shipping calculated at checkout. </p>
        <p className='y-2'>
  ...
  tracking-wide'>
      ...
      <section>
        {variant?.details?.length > 0 &&
          <div >
            ...
            <p>
              Because each strainer is hand-carved, you may notice slight variations in shape and size. These unique differences are what make every strainer special and full of character.
            </p>
          </div>
        }
      </section>
      ...
      <section>
        ...
        <p>
          Gently rinse with warm water and mild soap after use. Avoid soaking for long periods to preserve the bamboo's natural beauty. Dry thoroughly before storing.
      ...
      <section>
        ...
        <p>
          Please expect some slight inperfections as every piece is hand made and hand glazed which makes it unique to you.
        </p>
      </section>
      <section>
        ...
        <p>
          I’d love to see how you style your tea strainer alongside my ceramics! Tag me on Instagram @curlypottery to share your photos, or follow along for behind-the-scenes updates and inspiration.
        </p>
      </section>
