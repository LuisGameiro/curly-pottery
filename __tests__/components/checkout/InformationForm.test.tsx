...
describe("InformationForm", () => {
  function renderGuest(onComplete = jest.fn(), initialData: any = {}, isLoggedIn = false) {
    return render(
      <InformationForm onComplete={onComplete} initialData={initialData} isLoggedIn={isLoggedIn} />,
    );
  }
  ...
  it("prefills email and phone from initialData", () => {
    ...
    const email = screen.getByPlaceholderText(/Email Address/i) as HTMLInputElement;
  ...
  it("submits form and passes FormData to onComplete", () => {
    ...
    fireEvent.change(screen.getByPlaceholderText(/Postcode/i), { target: { value: "E1 1AA" } });

    fireEvent.submit(screen.getByRole("button", { name: /Continue to Shipping/i }).closest("form")!);
